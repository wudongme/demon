...
public class Timestamp {
	...
	// ----------------------------------适配当前时区---------------------------------------
	// 根据时区创建 ZoneOffset 实例
	private static ZoneOffset CURRENT_ZONE_OFFSET = ZoneId.systemDefault().getRules().getOffset(java.time.Instant.now());
	// ----------------------------------适配当前时区---------------------------------------
    ...
    public static long toEpochMillis(Object value, TemporalAdjuster adjuster) {
		...
		// ----------------------------------适配当前时区---------------------------------------
        return dateTime.toInstant(CURRENT_ZONE_OFFSET).toEpochMilli();
		// ----------------------------------适配当前时区---------------------------------------
    }
